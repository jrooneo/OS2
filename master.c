...
int main(int argc, char **argv)
{
	...
	int i, status;
	char processNum[3], countHolder[2];
	...
	for(i=0;i <= consumerCount; i++){
		...
		if(child[i] == 0){								//if this process is a child
			if(i == consumerCount){									//and this is the last iteration
				execl("./producer",(const char *) 0);	//Then replace the process with producer
			}else{
				sprintf(processNum, "%d", i);
				sprintf(countHolder, "%d", consumerCount);
				execl("./consumer","consumer", processNum, countHolder,(const char *) 0);	//Else we will replace with consumer
			}
		}
	}
	
	for(i=0;i <= 19; i++){ //loop through all 19 possible children 
